...
def random_interviewer() #sample
  interviewers_quant = INTERVIEWERS.size
  interviewers_quant = interviewers_quant - 1
  random = rand(interviewers_quant)+1
  INTERVIEWERS[random][:name] 
end

CANDIDATES.each do |candidate| #map maybe?

  interviewer1 = random_interviewer
  interviewer2 = random_interviewer

  # Checks if the the 2 interviewers are the same person
  while interviewer1 == interviewer2
    interviewer2 = random_interviewer
  end
